...
def _appel_api(methode: str, url_base: str, cle_api: str, chemin: str, **kwargs):
    try:
        ...
        st.error(f"Impossible de joindre l'API à l'adresse « {url_base} ». Vérifiez l'URL dans la barre latérale.")
        return None
    ...
    if reponse.status_code == 401:
        st.error("Clé API refusée. Vérifiez la clé saisie dans la barre latérale.")
        return None
    if not reponse.ok:
...
def onglet_tickets(url_base: str, cle_api: str) -> None:
    ...
    # Cliquer ce bouton n'a pas d'effet propre : sa seule fonction est de
    # provoquer une nouvelle execution du script (comportement natif de
    # tout bouton Streamlit), qui refait l'appel /tickets ci-dessous.
    st.button("🔄 Actualiser la liste")
    st.caption(
...
def _afficher_detail_ticket(url_base: str, cle_api: str, ticket_id: str) -> None:
    ...
    if ticket["degrade"]:
        st.warning("⚠️ Prédiction en mode dégradé : une ou plusieurs mesures capteur manquaient et ont été imputées.")
    ...
    with st.expander("Facteurs déclencheurs (explicabilité SHAP)"):
        ...
        if st.button("Calculer l'explication", key=f"expliquer_{ticket_id}"):
    ...
    if ticket["statut"] == "en_attente_validation_technicien_senior":
        ...
        if st.button("✅ Valider ce ticket (technicien senior)", key=f"valider_{ticket_id}"):
        ...
        if st.button("Clôturer le ticket", key=f"cloturer_{ticket_id}"):
            resultat = _appel_api(
...
def onglet_monitoring(url_base: str, cle_api: str) -> None:
    ...
    if derive is not None:
        ...
        for nom_modele, libelle in [("isolation_forest", "Détection d'anomalies"), ("random_survival_forest", "Durée de vie résiduelle")]:
            ...
            st.write(f"**{libelle}** — {resultat['n_observations']} observations dans la fenêtre glissante")
            if resultat["derive_confirmee"]:
                st.error("Dérive significative détectée — réentraînement recommandé.")
...
def onglet_a_propos() -> None:
    ...
    st.markdown(
        """
Cette interface consomme l'API de prédiction de maintenance décrite dans ce dépôt.

**Portée des données** — ce système ne traite que des mesures capteur et des attributs machine
(`machine_id`, valeurs de capteurs agrégées). Il ne reçoit et ne traite **jamais** de donnée liée à un
opérateur, une équipe ou un poste : la non-discrimination entre équipes/postes est garantie
structurellement par l'absence de ces variables dans les modèles, pas par une règle ajoutée après coup.

**Explicabilité** — chaque ticket peut afficher les facteurs ayant le plus contribué à l'alerte
(méthode SHAP), pour que le technicien comprenne la décision plutôt que de la subir.

**Supervision humaine** — un ticket de criticité élevée reste en attente tant qu'un technicien senior
ne l'a pas validé explicitement : ce système ne déclenche jamais seul une action corrective.

**Accessibilité** — cette interface n'utilise que des composants natifs de Streamlit (aucun HTML/JS
injecté), avec un label explicite sur chaque champ et une information critique toujours donnée en texte
(jamais par la couleur seule). Limite connue et assumée : les tableaux interactifs (`st.dataframe`) ont
des limites documentées de compatibilité avec les lecteurs d'écran côté Streamlit — l'information
essentielle de chaque ticket est donc dupliquée en texte simple au-dessus de ces tableaux plutôt que de
n'exister que dans le tableau.
        """
    )
